...
LoadDynamic0(void)
{
	 ...
	 * margin so an entry we can't unlink can't OOM init. */
	{
		CPlayer *p;
		int initialCount = 0;
		int iterCap;
		...
		while ((p = g_PlayerList.head) != NULL) {
			if (drained++ >= iterCap) {
				fprintf(stderr, "LoadDynamic0: player drain exceeded cap (%d initial + 16); could not unlink %p (serial 0x%08x) - aborting\n",
